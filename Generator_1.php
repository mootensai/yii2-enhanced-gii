 ...
 * @since 2.0
 */
class Generator_1 extends \yii\gii\Generator
{
    public $db = 'db';
    public $nsModel = 'app\models';
    public $nsTrait = 'app\traits';
    public $tableName;
    public $modelClass;
    ...
    public $useUUID;
    public $isOptimisticLock = 1;
    public $optimisticLockColumn;
    ...
    {
        return 'Enhanced Model Generator';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'nsModel', 'nsTrait', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'], 'filter', 'filter' => 'trim'],
            [['nsModel', 'nsModel', 'queryNs'], 'filter', 'filter' => function($value) { return trim($value, '\\'); }],

            [['db', 'nsModel', 'nsModel', 'tableName', 'baseClass', 'queryNs', 'queryBaseClass'], 'required'],
            [['db', 'modelClass', 'queryClass'], 'match', 'pattern' => '/^\w+$/', 'message' => 'Only word characters are allowed.'],
            [['nsModel', 'baseClass', 'queryNs', 'queryBaseClass'], 'match', 'pattern' => '/^[\w\\\\]+$/', 'message' => 'Only word characters and backslashes are allowed.'],
            [['tableName'], 'match', 'pattern' => '/^(\w+\.)?([\w\*]+)$/', 'message' => 'Only word characters, and optionally an asterisk and/or a dot are allowed.'],
            [['db'], 'validateDb'],
            [['nsModel', 'nsModel', 'queryNs'], 'validateNamespace'],
            ...
            [['enableI18N', 'isOptimisticLock'], 'boolean'],
            [['messageCategory','optimisticLockColumn'], 'validateMessageCategory', 'skipOnEmpty' => false],
        ]);
    }
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            'nsModel' => 'Namespace for Model',
            'nsTrait' => 'Namespace for Trait',
            ...
            'useUUID' => 'Use UUID as primary key',
            'isOptimisticLock' => 'Use Optimistic Lock'
        ]);
    }
    ...
    {
        return array_merge(parent::stickyAttributes(), ['nsModel', 'db', 'baseClass', 'generateRelations', 'generateLabelsFromComments', 'queryNs', 'queryBaseClass']);
    }
    ...
    {
        $this->nsModel = ltrim($this->nsModel, '\\');
        $path = Yii::getAlias('@' . str_replace('\\', '/', $this->nsModel), false);
        if ($path === false) {
            $this->addError('nsModel', 'Namespace must be associated with an existing directory.');
        }
    }
